...
func (d *Device) Setup(Resources *res.Resources, forwarder *f.Forwarder) {
	...
	// Ensure RX windows exist when loaded from JSON with "rxs": [] (avoid index out of range in ClassA.SendData)
	if len(d.Info.RX) < 2 {
		params := d.Info.Configuration.Region.GetParameters()
		freqRX2 := params.FrequencyRX2
		if freqRX2 == 0 {
			freqRX2 = 869525000
		}
		drRX2 := uint8(params.DataRateRX2)
		ch0 := d.Info.Configuration.Channels[0]
		d.Info.RX = make([]features.Window, 2)
		d.Info.RX[0] = features.Window{
			Delay:        time.Second,
			DurationOpen: 3 * time.Second,
			DataRate:     d.Info.Status.DataRate,
			Channel:      channels.Channel{FrequencyDownlink: ch0.FrequencyDownlink},
		}
		d.Info.RX[1] = features.Window{
			Delay:        time.Second,
			DurationOpen: 3 * time.Second,
			DataRate:     drRX2,
			Channel:      channels.Channel{FrequencyDownlink: freqRX2},
		}
	}
