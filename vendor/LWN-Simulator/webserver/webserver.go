...
func NewWebServer(config *models.ServerConfig, controller cnt.SimulatorController) *WebServer {
	...
	router.RedirectFixedPath = false // avoid any redirect on path normalization (prevents ERR_TOO_MANY_REDIRECTS)
	...
	// Serve SPA and static files from ROOT (/) to avoid /dashboard/ redirect/cache issues.
	serveStatic := func(c *gin.Context, path string) {
		...
		tryPath := path
		if !strings.HasPrefix(tryPath, "/") {
			tryPath = "/" + tryPath
		}
		f, err := staticFS.Open(tryPath)
		if err != nil {
			f, err = staticFS.Open(path)
		}
		if err != nil {
			for _, try := range []string{"/index.html", "index.html"} {
				f, err = staticFS.Open(try)
				if err == nil {
					path = "index.html"
					break
				}
			...
			path = strings.TrimPrefix(tryPath, "/")
		}
		if err != nil || f == nil {
			c.String(http.StatusNotFound, "Not Found")
			return
	...
	// NoRoute: redirect /dashboard* to /, then serve SPA/static from root (avoids Gin conflict with /dashboard/*path)
	router.NoRoute(func(c *gin.Context) {
		p := c.Request.URL.Path
		if p == "/dashboard" || p == "/dashboard/" || strings.HasPrefix(p, "/dashboard/") {
