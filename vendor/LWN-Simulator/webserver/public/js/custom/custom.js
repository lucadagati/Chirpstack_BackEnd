...
function SetParameters(code, loadDevice, dev){
    ...
    socket.emit("get-regional-parameters", code,(data)=>{
        ...
        $("#label-datarate-rx2").html("Default value is "+(data && data.dataRateRX2 != null ? data.dataRateRX2 : 0)+". "+table);
        $("#label-datarate-uplink").html(table);

        //da gestire il payload

        if (loadDevice && dev && dev.info){
            $("#dr-offset-rx1").val(dev.info.configuration ? dev.info.configuration.rx1DROffset : 0);
            var rxs1 = dev.info.rxs && dev.info.rxs[1];
            $("#datarate-rx-2").val(rxs1 && (rxs1.dataRate != null) ? rxs1.dataRate : 0);
            $("#datarate-uplink").val(dev.info.configuration ? dev.info.configuration.dataRate : 5);
        }
...
function LoadDevice(dev){
    ...
    //class A (guard: rxs may be empty or missing delay/durationOpen when loaded from JSON)
    var rxs0 = dev.info.rxs && dev.info.rxs[0];
    var rxs1 = dev.info.rxs && dev.info.rxs[1];
    var cfg = dev.info.configuration || {};
    var status = dev.info.status || {};
    var infoUplink = status.infoUplink || {};
    $("[name=input-rx-1-delay]").val(rxs0 && (rxs0.delay != null) ? rxs0.delay : 1000);
    $("[name=input-rx-1-duration]").val(rxs0 && (rxs0.durationOpen != null) ? rxs0.durationOpen : 3000);
    $("[name=input-rx-2-delay]").val(rxs1 && (rxs1.delay != null) ? rxs1.delay : 1000);
    $("[name=input-rx-2-duration]").val(rxs1 && (rxs1.durationOpen != null) ? rxs1.durationOpen : 3000);
    $("[name=input-frequency-rx-2]").val(rxs1 && rxs1.channel ? rxs1.channel.freqDownlink : 869525000);
    $("[name=input-ackTimeout]").val(cfg.ackTimeout != null ? cfg.ackTimeout : 5);
    $("#dr-offset-rx1").val(cfg.rx1DROffset != null ? cfg.rx1DROffset : 0);

    //ClassB,C
    $("#classB-dev").prop("checked", !!cfg.supportedClassB);
    $("#classC-dev").prop("checked", !!cfg.supportedClassC);

    //frame settings
    $("[name=input-fport]").val(infoUplink.fport != null ? infoUplink.fport : 1);
    $("[name=input-retransmission]").val(cfg.nbRetransmission != null ? cfg.nbRetransmission : 0);
    $("[name=input-fcnt]").val(infoUplink.fcnt != null ? infoUplink.fcnt : 0);
    $("#datarate-uplink").val(cfg.dataRate != null ? cfg.dataRate : 5);

    $("[name=input-validate-counter]").prop("checked", !!cfg.disableFCntDown);
    if (!cfg.disableFCntDown)
        $("[name=input-fcnt-downlink]").val((status.fcntDown != null) ? status.fcntDown : 0);

    //features
    $("[name=input-ADR]").prop("checked", !!cfg.supportedADR);
    $("[name=input-range]").val(cfg.range != null ? cfg.range : 10000);
